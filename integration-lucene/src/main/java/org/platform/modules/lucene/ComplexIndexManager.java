...
/** 基于内存、文件混合索引的操作*/
public class ComplexIndexManager extends AbstrIndexManager {
	...
	public void insert(Object... objects) {
		...
		try {
			if (isMerge) {
				IndexController.getInstance().obtainIndexWriter(IIndex.RAM_TEMP).addDocuments(documents);
			} else {
	...
	public void merge(Directory...directories) {
		try {
			...
			IndexController.getInstance().obtainIndexWriter(IIndex.RAM_TEMP).commit();
			IndexController.getInstance().obtainIndexWriter(IIndex.FILE).addIndexes(
					IndexController.getInstance().obtainIndexWriter(IIndex.RAM_TEMP).getDirectory());
			IndexController.getInstance().obtainIndexWriter(IIndex.FILE).commit();
			IndexController.getInstance().closeIndexWriter(IIndex.RAM_TEMP);
	...
	public QueryResult<?> readByCondition(QueryCondition condition) {
		...
		try {
			...
			IndexController.getInstance().obtainIndex(indexType).releaseIndexSearcher(indexSearcher);
		}
