...
$.app = {
    ...
    initIndex: function () {
        ...
        var pollingUrl = contextURL + "/admin/polling";
    ...
    initCommonBtn : function() {
        $(".btn-view-info,.btn-change-password").click(function() {
            ...
            if(a.is(".btn-view-info")) {
                url = contextURL +"/admin/sys/user/loginUser/viewInfo";
            } else if(a.is(".btn-change-password")) {
                url = contextURL + "/admin/sys/user/loginUser/changePassword";
            }
            setTimeout(function() {
        ...
        $(".btn-view-message,.btn-message").click(function() {
            var url = contextURL + "/admin/personal/message";
            setTimeout(function() {
                $.tabs.activeTab($.tabs.nextCustomTabIndex(), "我的消息", url, true)
            }, 0);
        });
        $(".btn-view-notice").click(function() {
            var url = contextURL + "/office/personal/notice/list?read=false";
            setTimeout(function() {
        ...
        $(".btn-view-worklist,.btn-view-work").click(function() {
            var $that = $(this);
            var url = contextURL + "/office/personal/worklist";
            setTimeout(function() {
    ...
    initNotification : function() {
        ...
        var markReadUrl = contextURL + "/admin/personal/notification/markRead?id=";
        ...
        return {
            update : function(dataList) {
                ...
                for(var i = 0, l = dataList.length; i < l; i++) {
                    ...
                    if(!data.read) {
                        hasUnread = true;
                        data.title = data.title.replace("{contextURL}", contextURL);
                        data.content = data.content.replace("{contextURL}", contextURL);
                    }
                }
    ...
    waiting : function(message, isSmall) {
        ...
        message = '<img src="' + contextURL + '/static/images/loading.gif" '+ (isSmall ? "width='20px'" : "") +'/> ' + message;
        if(!isSmall) {
...
//            });
    ...
    initCalendar : function() {
        ...
        var calendar = $('#calendar').fullCalendar({
            ...
            events: contextURL + "/admin/personal/calendar/load",
        ...
        function openNewCalendarForm(start, end) {
            var url = contextURL + "/admin/personal/calendar/new";
            if(start) {
        ...
        function moveCalendar(event) {
            var url = contextURL + "/admin/personal/calendar/move";
            var id = event.id;
        ...
        function viewCalendar(event) {
            var url = contextURL + "/admin/personal/calendar/view/" + event.id;
        ...
        $("body").on("click", ".btn-delete-calendar", function() {
            ...
            $.app.confirm({
                ...
                message : '确认删除提醒事项吗？',
                ok : function() {
                    var url = contextURL + "/admin/personal/calendar/delete?id=" + $this.data("id");
                    $.post(url, function() {
                        calendar.fullCalendar("refetchEvents");
    ...
    removeContextPath : function(url) {
        if(url.indexOf(contextURL) == 0) {
            return url.substr(contextURL.length);
        }
        return url;
    }
    ,
...
$.parentchild = {
     ...
     *     {
                ...
                updateUrl : "${contextURL}/showcase/parentchild/parent/child/{id}/update" 修改时url模板 {id} 表示修改时的id,
                deleteUrl : "${contextURL}/showcase/parentchild/parent/child/{id}/delete  删除时url模板 {id} 表示删除时的id,
            }
     * @param options
     * @return {boolean}
     */
     ...
     *     createUrl : "${contextURL}/showcase/parentchild/parent/child/create",
     *     updateUrl : "${contextURL}/showcase/parentchild/parent/child/{id}/update" 修改时url模板 {id} 表示修改时的id,
     *     deleteUrl : "${contextURL}/showcase/parentchild/parent/child/{id}/delete  删除时url模板 {id} 表示删除时的id,
     * }
     */
